...
namespace FYP1System.Pages.Evaluator.Proposals
{
    ...
    public class DownloadModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync(int id)
        {
            ...
            if (assignment == null || string.IsNullOrEmpty(assignment.Proposal.FilePath))
            {
                TempData["ErrorMessage"] = "Proposal file not found or not assigned to you.";
                return RedirectToPage("/Evaluator/Proposals");
            }

            // Build the full file path
            var uploadsPath = Path.Combine(_environment.WebRootPath, "uploads");
            var filePath = Path.Combine(uploadsPath, assignment.Proposal.FilePath);
            ...
            var fileExtension = Path.GetExtension(assignment.Proposal.FilePath);
            var fileName = $"{assignment.Proposal.Student.StudentId}_{assignment.Proposal.Title.Replace(" ", "_").Replace("/", "_")}_Proposal{fileExtension}";
